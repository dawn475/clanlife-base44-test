...
export default function CatCard({ cat }) {
  ...
  const isLeader = cat.rank === "leader";
  const leaderLevel = cat.leader_level || 1;
  const leaderExp = cat.leader_exp || 0;
  const expToNext = leaderLevel * 100;
              ...
              {isLeader && (
                <div className="mt-1.5">
                  <div className="flex items-center justify-between text-[9px] text-muted-foreground mb-0.5">
                    <span>⭐ Lv.{leaderLevel}</span>
                    <span>{leaderExp}/{expToNext} EXP</span>
                  </div>
                  <div className="h-1 bg-muted rounded-full overflow-hidden">
                    <div
                      className="h-full bg-yellow-500 rounded-full transition-all"
                      style={{ width: `${Math.min(100, (leaderExp / expToNext) * 100)}%` }}
                    />
                  </div>
                </div>
              )}
